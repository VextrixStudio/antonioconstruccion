...
export default function Nav() {
  ...
  // Efecto para detectar el scroll y cambiar la opacidad del nav
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        `}>
          ...
          {/* DESKTOP NAV - Espaciado Apple Style */}
            ...
            <div className="flex gap-8">
              {links.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-[10px] font-bold uppercase tracking-[0.25em] text-zinc-400 transition-colors hover:text-white"
                >
                  {link.name}
                </Link>
              ))}
      ...
      {/* MOBILE MENU - GRID SYSTEM (Sin Framer Motion) */}
      <div
          ...
          <div className="grid grid-cols-1 gap-8">
            {links.map((link, i) => (
              <Link
                ...
                className={`text-4xl font-bold tracking-tighter text-zinc-100 transition-all duration-500 transform ${
